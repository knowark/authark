import json
from authark.application.domain.models import Token, Dominion
from authark.application.domain.common import AnonymousUser


async def test_access_service_generate_token(access_service) -> None:
    user = AnonymousUser(
        id='U1', username='johndoe', name="John Doe",
        email='johndoe', tid='T1', tenant='default')
...
async def test_access_service_generate_token_payload(access_service) -> None:
    user = AnonymousUser(
        id='U1', username='johndoe', name="John Doe", email='johndoe',
        tid='T1', organization='Default', tenant='default')
    dominion = Dominion(id='D1', name='Data Server')
...
async def test_access_service_generate_token_roles(access_service) -> None:
    user = AnonymousUser(
        id='1', username='johndoe', email='johndoe',
        tid='1', organization='Default', tenant='default')
