...
from authark.application.utilities import QueryParser, UserCreationError
...
async def test_generate_refresh_token(auth_coordinator: AuthCoordinator) -> None:
    user_id = '1'
    client = 'mobile'
    refresh_token = await auth_coordinator._generate_refresh_token(user_id, client)
    credential_repository = cast(MemoryCredentialRepository,
                                 auth_coordinator.credential_repository)
...
async def test_auth_coordinator_register(
    ...
    user_dict = {"username": "mvp", "email": "mvp@gmail.com",
                 "password": "PASS4"}
    user_dict = await auth_coordinator.register(user_dict)
    credential_repository = cast(MemoryCredentialRepository,
                                 auth_coordinator.credential_repository)
...
async def test_auth_coordinator_register_username_special_characters_error(
    ...
    with raises(UserCreationError):
        user_dict = {"username": "mvp@gmail.com", "email": "mvp@gmail.com",
                     "password": "PASS4"}
        await auth_coordinator.register(user_dict)
...
async def test_auth_coordinator_deregister(
    ...
    unregistered = await auth_coordinator.deregister(user.id)
    credential_repository = cast(MemoryCredentialRepository,
                                 auth_coordinator.credential_repository)
    user_repository = cast(MemoryUserRepository,
                           auth_coordinator.user_repository)
