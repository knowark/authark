...
async def test_management_coordinator_create_dominion(management_coordinator):
    dominion_dict = dict(
        id='abc001', name='HR Server', url='https://hr.example.com')
    await management_coordinator.create_dominion(dominion_dict)
...
async def test_management_coordinator_remove_dominion(management_coordinator):
    dominion_id = '1'
    await management_coordinator.remove_dominion(dominion_id)
    assert len(management_coordinator.dominion_repository.data[
        'default']) == 0


async def test_management_coordinator_create_role(management_coordinator):
    role_dict = dict(
        id='2', name='admin', dominion_id='abc001',
        description='Administrator')
    await management_coordinator.create_role(role_dict)
...
async def test_management_coordinator_remove_role(management_coordinator):
    role_id = '1'
    await management_coordinator.remove_role(role_id)
    assert len(management_coordinator.role_repository.data['default']) == 0


async def test_management_coordinator_assign_role(management_coordinator):
    user_id = '2'
    role_id = '1'
    await management_coordinator.assign_role(user_id, role_id)
...
async def test_management_coordinator_deassign_role(management_coordinator):
    ranking_id = '1'
    await management_coordinator.deassign_role(ranking_id)
