...
def test_management_coordinator_instantiation(management_coordinator) -> None:
    assert management_coordinator is not None


async def test_management_coordinator_create_dominion(
        management_coordinator) -> None:
    ...
    assert len(
        management_coordinator.dominion_repository.data['default']) == 2
...
async def test_management_coordinator_assign_role(
    ...
    await management_coordinator.assign_role(ranking_dicts)
